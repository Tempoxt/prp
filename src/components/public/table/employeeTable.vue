<template>
  <el-col v-if="table && showList.length" ref="wrap" class="table">
      ...
      <div v-for="(item, index) in table.header" :key="index">
        <div v-for="(item, index) in item" :key="index" class="leftItem">
          <el-button-group v-if="item.type === 'buttonGroup'">
            <template v-for="(i, d) in item.data">
              <el-button
                ...
                :disabled="
                  i.disabled ||
                  (i.isSelect && table.selectArr.length === 0) ||
                  (i.isSelects &&
                    (table.selectArr.length === 0 ||
                      table.selectArr.length > 1))
                "
                :type="i.color"
          ...
          <div v-if="item.type === 'manySearch'">
            <el-input
              :placeholder="
                typeof item.holder === 'object'
                  ? item.holder[item.optionValue]
                    ? item.holder[item.optionValue]
                    : ''
                  : item.holder
              "
              ...
              @blur="search(item.optionValue, item.value)"
              @keypress.enter.native="$event.target.blur()"
            >
              <el-select
    ...
    <div
      ...
      class="table-main"
      :class="[
        'header' in table ? 'headerPadding' : '',
        'footer' in table ? 'footerPadding' : '',
      ]"
      :style="tableHeight"
    >
      <el-table
        :data="table.main.body"
        :row-class-name="table.main.rowClass"
        :row-key="table.row_key ? table.row_key : undefined"
        :tree-props="table.tree_props ? table.tree_props : { hasChildren: 'hasChildren', children: 'children' }"
        tooltip-effect="dark"
        style="width: 100%"
      ...
      >
        ...
        <template v-for="(column, num) of table.main.head">
          ...
          <el-table-column
            ...
            :width="
              column.width ? column.width : table.width ? table.width : ''
            "
            :fixed="column.fixed"
          >
            <template slot-scope="scope">
              <el-tooltip
                ...
                effect="dark"
                :disabled="
                  scope.row[column.keys] === '' ||
                  scope.row[column.keys] === null ||
                  showTip
                "
                :content="
                  scope.row[column.keys] === '' ||
                  scope.row[column.keys] === null
                    ? '-'
                    : String(scope.row[column.keys])
                "
                placement="bottom-start"
              >
                <span @mousemove="isOverFlow">{{
                  scope.row[column.keys] === "" ||
                  scope.row[column.keys] === null
                    ? "-"
                    : scope.row[column.keys]
            ...
            <template slot-scope="scope">
              <el-tooltip
                ...
                :content="
                  ...
                  scope.row[column.keys] === null
                    ? '-'
                    : String(column.rules[scope.row[column.keys]])
                "
                placement="bottom"
              >
                <span @click="test([scope.row, column])">{{
                  scope.row[column.keys] === "" ||
                  scope.row[column.keys] === null
                    ? "-"
                    : column.rules[scope.row[column.keys]]
                }}</span>
              </el-tooltip>
              <!-- <span>{{scope.row[column.keys] === '' || scope.row[column.keys] === null ? '-' : scope.row[column.keys]}}</span> -->
            </template>
                ...
                :disabled="
                  'disabled' in column
                    ? typeof column.disabled === 'function'
                      ? column.disabled(scope)
                      : column.disabled
                    : false
                "
                @blur="column.blur ? handlerCb(column.blur, scope.row) : ''"
              ></el-input>
            </template>
          </el-table-column>
          <el-table-column
            v-if="column.type === 'inputTip' && showList[num].check"
            :label="column.title"
            :key="column.title"
            :width="
              column.width ? column.width : table.width ? table.width : ''
          ...
          >
            <template slot-scope="scope">
              <el-tooltip
                ...
                placement="right"
                :disabled="
                  'tipDisabled' in column
                    ? typeof column.tipDisabled === 'function'
                      ? column.tipDisabled(scope)
                      : column.tipDisabled
                    : false
                "
              >
          ...
          >
            <template slot-scope="scope">
              <el-select
                ...
                :filter-method="column.filterMethods ? (v) => column.filterMethods(v, scope) : null"
                :disabled="
                  'disableRule' in column
                    ? typeof column.disableRule === 'function'
                      ? column.disableRule(scope)
                      : column.disableRule
                    : false
                "
                @change="column.func ? handlerCb(column.func, scope) : null"
              >
                <template v-if="column.single_option">
                  <el-option
                    v-for="(it, ind) in scope.row[column.keys + '_options']"
                    :key="ind"
                    :label="it.label"
                    :value="it.value"
                    :disabled="
                      'optionDisableRule' in column
                        ? typeof column.optionDisableRule === 'function'
                          ? column.optionDisableRule(scope, it.value)
                          : column.optionDisableRule
                        : false
                    "
                  >
                  </el-option>
          ...
          >
            <template slot-scope="scope">
              <el-select
                v-model="scope.row[column.keys]"
                @select="test(scope.row[column.keys])"
              >
                <el-option
                  v-for="(it, ind) in column.options"
                  :key="ind"
          ...
          >
            <template slot-scope="scope">
              <i
                class="icon iconfont"
                :class="[
                  scope.row[column.keys] in column.rules &&
                  scope.row[column.keys] !== null
                    ? column.rules[scope.row[column.keys]].icon
                    : 'icon-baoyangjilu',
                ]"
                :style="{
                  color:
                    scope.row[column.keys] in column.rules &&
                    scope.row[column.keys] !== null
                      ? column.rules[scope.row[column.keys]].color
                      : '#24ccff',
                  fontSize: '20px',
                }"
              />
          ...
          >
            <template slot-scope="scope">
              <el-upload
                ...
                :file-list="scope.row[column.keys]"
                :on-change="
                  (file, fileList) => column.handleFile(file, fileList, scope)
                "
              >
          ...
          >
            <template slot-scope="scope">
              <el-tag
                ...
                :type="
                  scope.row[column.keys] !== null
                    ? scope.row[column.keys] in column.options
                      ? column.options[scope.row[column.keys]].color
                      : ''
                    : ''
                "
              >
                {{
                  scope.row[column.keys] !== null
                    ? scope.row[column.keys] in column.options
                      ? column.options[scope.row[column.keys]].tip
                      : "--"
                    : "--"
                }}</el-tag
              >
          ...
          >
            ...
            <template slot-scope="scope">
              <el-checkbox
                :disabled="column.disabled ? column.disabled(scope.row) : false"
                @change="
                  isAll(column.keys, scope.row[column.keys], scope.$index)
                    ? (column.all = true)
                    : (column.all = false)
                "
                v-model="scope.row[column.keys]"
